...
%     if ~isfield(data(ii), 'mag_angular_depth') % don't recompute to save time
        data(ii).mag_angular_depth = MagneticGuidanceGetAngleFromVideo(filepaths(ii).mag.vid, angle_smooth_span);
%     end

%     if ~isfield(data(ii), 'nomag_mea_angular_depth')
...
%     if ~isfield(data(ii), 'mag_mea_interp_angdepth')
...
%     if ~isfield(data(ii), 'nomag_mea_interp_angdepth')
...
% colors = distinguishable_colors(2*length(data));
colors = distinguishable_colors(4);

for ii=1:2
% for ii=1:length(data)
    plot( data(ii).mag.depth_insertion, data(ii).mag_interp_angdepth, 'Color',colors(ii,:), 'LineStyle','--');
    plot( data(ii).nomag_mea.depth_insertion, data(ii).nomag_mea_interp_angdepth, 'Color',colors(ii*2,:), 'LineStyle',':');
end
...
% for ii=1:length(data)
    ...
    h(ii).mag(3) = plot( data(ii).mag_interp_angdepth, data(ii).mag.Fz_smooth, 'Color', [0,0,1, 1.0], 'LineWidth', 2);
                     ...
                     plot( data(ii).nomag_mea_interp_angdepth, data(ii).nomag_mea.Fx,        'Color', [1,0,0, 0.3], 'LineWidth', 1);
    h(ii).nomag(1) = plot( data(ii).nomag_mea_interp_angdepth, data(ii).nomag_mea.Fx_smooth, 'Color', [1,0,0, 1.0], 'LineWidth', 2);
                     plot( data(ii).nomag_mea_interp_angdepth, data(ii).nomag_mea.Fy,        'Color', [0,1,0, 0.3], 'LineWidth', 1);
    h(ii).nomag(2) = plot( data(ii).nomag_mea_interp_angdepth, data(ii).nomag_mea.Fy_smooth, 'Color', [0,1,0, 1.0], 'LineWidth', 2);
                     plot( data(ii).nomag_mea_interp_angdepth, data(ii).nomag_mea.Fz,        'Color', [0,0,1, 0.3], 'LineWidth', 1);
    h(ii).nomag(3) = plot( data(ii).nomag_mea_interp_angdepth, data(ii).nomag_mea.Fz_smooth, 'Color', [0,0,1, 1.0], 'LineWidth', 2);

    figure(2)
    plot( data(ii).mag_interp_angdepth, data(ii).mag.Fmag,        'Color', [0,0,1, 0.3], 'LineWidth', 1);
    plot( data(ii).mag_interp_angdepth, data(ii).mag.Fmag_smooth, 'Color', [0,0,1, 0.3], 'LineWidth', 1);
    plot( data(ii).nomag_mea_interp_angdepth, data(ii).nomag_mea.Fmag,        'Color', [0,0,0, 0.3], 'LineWidth', 1);
    plot( data(ii).nomag_mea_interp_angdepth, data(ii).nomag_mea.Fmag_smooth, 'Color', [0,0,0, 0.3], 'LineWidth', 1);
end
