function transformed_points = transform_between_coordinate_systems(input_points,dim,vsize,input_orientation,output_orientation)
...
for n_point = 1:size(input_points,2)
    ...
    for axis = 1:length(dim)
        if input_orientation(axis)~=output_orientation(axis)
            transformed_points(axis,n_point) = vsize(axis)*(dim(axis)-1)...
                -input_points(axis,n_point);
        else
